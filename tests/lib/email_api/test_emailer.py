...
def instance():
    ...
    return Emailer(mock_smtp_account, mock_template_handler)
...
def test_send_emails(mock_smtp_ssl, mock_build_email, instance):
    ...
    mock_smtp_ssl.assert_called_once_with(
        instance._smtp_account.server,
        instance._smtp_account.port,
        timeout=60,
    )
...
def test_attach_files(mock_mime_application, mock_file, instance):
    ...
    res = instance._attach_files(mock_msg, mock_filepaths)
...
def test_attach_files_invalid_fp(_, instance):
    ...
    with pytest.raises(RuntimeError):
        instance._attach_files(mock_msg, mock_filepaths)


@patch("email_api.emailer.Emailer.build_email_body")
def test_build_email_header_fields(
    mock_build_email_body, instance
):
    """
...
def test_build_email_with_attachments(
    ...
    mock_attach_files.assert_called_once_with(mock_mime_multipart.return_value, mock_email_params.attachment_filepaths)


@pytest.mark.parametrize("as_html", [True, False])
@patch("email_api.emailer.MIMEText")
def test_build_email_body(mock_mime_text, as_html, instance):

    template_details_1 = MagicMock()
    template_details_2 = MagicMock()
    template_list = [template_details_1, template_details_2]

    instance._template_handler.render_html_template.side_effect = [
        "template-render-html-1\n",
        "template-render-html-2\n",
    ]

    instance._template_handler.render_plaintext_template.side_effect = [
    ...
    if as_html:
        assert instance._template_handler.render_html_template.call_count == 2
        assert instance._template_handler.render_plaintext_template.call_count == 0
        assert instance._template_handler.render_html_template.call_args_list == [call(template_details_1), call(template_details_2)]
        mock_mime_text.assert_called_once_with(
            "template-render-html-1\ntemplate-render-html-2\n", "html"
        )
    else:
        assert instance._template_handler.render_html_template.call_count == 0
        assert instance._template_handler.render_plaintext_template.call_count == 2
        assert instance._template_handler.render_plaintext_template.call_args_list == [call(template_details_1), call(template_details_2)]
        mock_mime_text.assert_called_once_with(
            "template-render-plaintext-1\ntemplate-render-plaintext-2\n",
            "plain",
            "utf-8",
