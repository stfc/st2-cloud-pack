...
@pytest.fixture
def instance(mock_template_metadata):
...
@pytest.mark.parametrize("mock_schema, mock_given_vals, expected_out",
    [
        (
...
@pytest.mark.parametrize("mock_schema, mock_given_vals",
    [
...
def test_get_template_file_invalid(instance):
    ...
    mock_template_env.get_template.side_effect = TemplateNotFound(
        "template-error"
    )
    with pytest.raises(EmailTemplateError):
...
@pytest.mark.parametrize("mock_template_name, mock_file_path_key, mock_template_params",
    [
        # "html and schema",
        (
            "mock-template",
...
@pytest.mark.parametrize("file_path_key",
    [
        "html_filepath"
        "plaintext_filepath"
    ]
