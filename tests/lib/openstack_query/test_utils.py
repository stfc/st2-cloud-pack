...
def test_get_timestamp_in_seconds(days, hours, minutes, seconds, total_seconds, mock_current_datetime):
...
def test_get_timestamp_in_seconds_invalid():
    utils.get_timestamp_in_seconds(
        days=0,
        hours=0,
        minutes=0,
        seconds=0
...
@parameterized([
    (1, 0, 0, 0, "2023-06-04 10:30:00", "%Y-%m-%d %H:%M:%S", True),  # prop timestamp is older by 1 day
    (0, 12, 0, 0, "2023-06-04 10:30:00 AM", "%Y-%m-%d %I:%M:%S %p", True),  # prop timestamp is older by 12 hours
    (1, 0, 0, 0, "2023-06-02", "%Y-%m-%d", False),  # prop timestamp is younger by 1 day
    (0, 0, 30, 0, "2023-06-04 10:00:00", "%Y-%m-%d %H:%M:%S", False),  # prop timestamp is equal
    (0, 0, 0, 0.5, "2023-06-04 10:30:00", "%Y-%m-%d %H:%M:%S", True),  # prop timestamp is older by 0.5 seconds
])
@patch("openstack_query.utils.get_current_time")
def test_prop_older_than(days, hours, minutes, seconds, string_timestamp, timestamp_format, expected_out, mock_current_datetime):
...
@parameterized([
    (1, 0, 0, 0, "2023-06-02 10:30:00", "%Y-%m-%d %H:%M:%S", True),  # prop timestamp is younger by 1 day
    (0, 12, 0, 0, "2023-06-03 10:30:00 AM", "%Y-%m-%d %I:%M:%S %p", True),  # prop timestamp is younger by 12 hours
    (1, 0, 0, 0, "2023-06-04", "%Y-%m-%d", False),  # prop timestamp is older by 1 day
    (0, 0, 30, 0, "2023-06-04 10:00:00", "%Y-%m-%d %H:%M:%S", False),  # prop timestamp is equal
    (0, 0, 0, 0.5, "2023-06-03 10:29:59", "%Y-%m-%d %H:%M:%S", True),  # prop timestamp is younger by .5 seconds
])
@patch("openstack_query.utils.get_current_time")
def test_prop_younger_than(days, hours, minutes, seconds, string_timestamp, timestamp_format, expected_out, mock_current_time):
