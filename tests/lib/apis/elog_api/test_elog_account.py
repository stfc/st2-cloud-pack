...
@pytest.fixture(name="mock_elog_accounts")
def mock_elog_accounts_fixture() -> List[Dict]:
    """
    ...
    return [
        {
            ...
            "username": "elog",
            "password": "pass",
            "elog_endpoint": "elog.test.com",
        }
    ]


@pytest.fixture(name="mock_pack_config")
def mock_pack_config_fixture(mock_elog_accounts):
    """Fixture sets up a mock pack config to test with"""
    return {"elog_accounts": mock_elog_accounts}


def test_from_dict():
    """
    Tests that from_dict() static method works properly
    this method should build a ElogAccount dataclass from a valid dictionary
    """
    mock_valid_kwargs = {
        "username": "user1",
        "password": "some-pass",
    ...
    res = ElogAccount.from_dict({**mock_valid_kwargs, **mock_invalid_kwargs})
    for key, val in mock_valid_kwargs.items():
        assert val == getattr(res, key)


def test_from_pack_config_valid(mock_elog_accounts, mock_pack_config):
    """
    Tests that from_pack_config() static method works properly
    this method should build a ElogAccount dataclass from a valid
    stackstorm pack_config and a elog_account_name
    """
    for mock_accounts in mock_elog_accounts:
        expected_attrs = dict(mock_accounts)
        expected_attrs.pop("name")

        res = ElogAccount.from_pack_config(mock_pack_config, "config1")
...
def test_from_pack_config_invalid_name(mock_pack_config):
    ...
    with pytest.raises(KeyError):
        ElogAccount.from_pack_config(mock_pack_config, "invalid-config")
...
def test_from_pack_config_invalid_pack():
    ...
    with pytest.raises(ValueError):
        ElogAccount.from_pack_config({}, "config1")
