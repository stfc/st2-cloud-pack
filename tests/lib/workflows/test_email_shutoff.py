...
from workflows.email_shutoff import send_user_email, query_shutoff_vms, main, prepare_user_server_email
...
def test_query_shutoff_vms_no_project_id():
    ...
    select_object.where.assert_called_once_with(QueryPresetsGeneric.EQUAL_TO, ServerProperties.SERVER_STATUS,value="SHUTOFF")

    where_object = select_object.where.return_value
    where_object.group_by.assert_called_once_with(ServerProperties.USER_ID)

    server_object.run.assert_called_once_with("openstack", as_admin=True, all_projects=True)
...
def test_query_shutoff_vms_project_id():
    ...
    server_object.run.assert_called_once_with("openstack", as_admin=True, from_projects=mock_project_id)

    user_query = server_object.then.return_value

    user_query.run.assert_called_once_with("openstack")
...
def test_query_server_get_user_details():
    ...
    user_query.run.assert_called_once()

    user_query.to_string.assert_called_once()

    assert res == user_query.to_string.return_value
...
def test_prepare_user_server_email():
    ...
    """
    fake_server_a = {'server_name': NonCallableMock(), 'user_email': 'example@example.com'}
    fake_server_b = {'server_name': NonCallableMock(), 'user_email': 'example@example.com'}
...
def test_main(send_user_email, prepare_user_server_email, query_shutoff_vms):
    ...
    main(mock_smtp_account, mock_project_id)

    query_shutoff_vms.assert_called_once_with(mock_project_id)

    assert prepare_user_server_email.call_count == 2

    prepare_user_server_email.assert_has_calls([call(mock_data["a"]), call(mock_data["b"])])

    assert send_user_email.call_count == 2
    expected_email_user_a = call(mock_smtp_account, mocked_user_a[0], mocked_user_a[1])
    expected_email_user_b = call(mock_smtp_account, mocked_user_b[0], mocked_user_b[1])
