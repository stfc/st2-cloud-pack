...
def mock_template_metadata_fixture():
    """
    Returns a mock template metadata dict for the jira API to use
    """
    return {
        "mock-template": {
...
@patch("jira_api.template_handler.safe_load")
...
def test_load_all_metadata_valid(mock_file, mock_yaml_load, instance):
    ...
    Reads yaml file containing metadata info set in JIRA_TEMPLATE_METADATA_FP
    """
...
@patch("jira_api.template_handler.TemplateHandler._get_template_file")
...
@patch("jira_api.template_handler.TemplateHandler._render_template")
def test_render_plaintext_template(mock_render_template, instance):
    ...
    """
    mock_template_details = JiraTemplateDetails(
