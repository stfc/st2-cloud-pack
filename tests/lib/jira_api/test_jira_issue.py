...
@pytest.mark.parametrize(
    "comment_text, internal_flag",
    [("This is a test comment", True), ("Public comment", False)],
)
def test_add_comment(mock_jira, mock_account, comment_text, internal_flag):
    """Tests add_comment with different internal flag values"""
    add_comment(mock_account, "ISSUE-123", comment_text, internal=internal_flag)
    mock_jira.add_comment.assert_called_once_with(
        "ISSUE-123", comment_text, internal_flag
    )
