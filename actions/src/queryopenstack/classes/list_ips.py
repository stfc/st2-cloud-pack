...
class ListIps(ListItems):
    ...
    def __init__(self, conn):
        ...
        self.criteria_func_dict = {
            ...
            "not project_name": lambda dict, args: self.conn.identity.find_project(dict["project_id"])[
                                                       "name"] not in args,
            "project_name_contains": lambda dict, args: any(
                arg in self.conn.identity.find_project(dict["project_id"])["name"] for arg in args),
            "project_name_not_contains": lambda dict, args: any(
                arg not in self.conn.identity.find_project(dict["project_id"])["name"] for arg in args)
        }
