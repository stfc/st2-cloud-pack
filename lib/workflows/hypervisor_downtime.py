...
def _get_number_of_hours(start_dt, end_time_str):
    ...
    # Note the value of end_time_str can have 2 different formats
    # and we do not know yet which one it is
    #   - it can be an absolute date and time
    #   - it can be number of hours and/or number of days
    # We need to check with format has been passed in order to decide
    # how to process it
    ...
    #
    # The regex uses the | (OR) operator to check two main patterns:
    #   ^(?=.*\d)(?:(\d+)d)?\s*(?:(\d+)h)?$ —> matches days first, then hours
    #   ^(?=.*\d)(?:(\d+)h)?\s*(?:(\d+)d)?$ —> matches hours first, then days
    #
    # Individual token explanations:
    #
    #   ^ and $ -> anchors that mark the start and end of the string,
    #              ensuring the entire input is validated
    #   (?=.*\d) -> a positive lookahead that ensures the string contains
    #               at least one digit (\d) somewhere.
    #               Without this, an empty string or just whitespace would count as a match
    #   (?:(\d+)d)? ->
    #       (\d+) -> a capturing group that matches one or more digits
    #                (the number of days or hours)
    #       d -> matches the literal character "d"
    #       (?:...)? -> makes the entire unit optional, so you can have just hours,
    #                   just days, or both
    #       \s* -> matches zero or more whitespace characters between the two units
