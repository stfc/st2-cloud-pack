...
def check_filter_func(func: Callable[[Any], bool], arg_names: List[str]) -> bool:
    ...
    for arg in arg_names:
        if arg not in func_all_args:
            raise TypeError("unexpected argument '%s" % arg)
    return True


def check_kwarg_mapping(
    kwarg_mapping: Callable[[Any], Dict[str, Any]], args: Dict[str, Any]
):
    try:
...
def prop_younger_than_or_equal_to(
    ...
    return not prop_older_than(prop, days, hours, minutes, seconds, prop_timestamp_fmt)
